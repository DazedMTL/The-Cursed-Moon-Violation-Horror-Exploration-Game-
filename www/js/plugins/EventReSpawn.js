...
(function () {
  ...
  Game_Map.prototype.spawnEventRandom = function (
    ...
    if (conditionMap.regionIds) {
      conditions.push(
        this.isErsCheckRegionId.bind(this, conditionMap.regionIds)
      );
    }
  ...
  Game_Map.prototype.getConditionalValidPosition = function (conditions) {
    ...
    if (!$gameSwitches.value(param.CertainSpawnSwitch) && tryCount > 0) {
      ...
      var positions = [];
      for (var ix = 0; ix < $dataMap.width; ++ix)
        for (var iy = 0; iy < $dataMap.height; ++iy) {
  ...
  const _Game_PrefabEvent_controlSelfVariable =
    Game_PrefabEvent.prototype.controlSelfVariable;
  Game_PrefabEvent.prototype.controlSelfVariable = function (
    index,
    type,
    operand,
    formulaFlg
  ...
  if (typeof Window_EventMiniLabel !== "undefined") {
    var _Window_EventMiniLabel_gatherDisplayData =
