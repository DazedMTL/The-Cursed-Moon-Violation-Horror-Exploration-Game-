...
(function () {
  ...
  Skill_Sequencer.prototype.update = function () {
    ...
    if (
      this._waitForMove ||
      this._waitForUserMove ||
      this._waitForUserJump ||
      this._waitForPose
    ) {
      return;
    }
    ...
    if (this._skill.sequence.length === 0) {
      if (!this._skill.moving) {
        ...
        Terrax_ABS_blast_x.push(
          this._skill.collider.center.x - $gameMap.tileWidth() / 2
        );
        Terrax_ABS_blast_y.push(
          this._skill.collider.center.y - $gameMap.tileHeight() / 2
        );
  ...
  QuasiABS.Manager.startAction = function (self, targets, item) {
    if (!item.animationTarget || targets.length === 0) {
      this.startAnimation(
        item.data.animationId,
        item.collider.center.x,
        item.collider.center.y
      );
    }
