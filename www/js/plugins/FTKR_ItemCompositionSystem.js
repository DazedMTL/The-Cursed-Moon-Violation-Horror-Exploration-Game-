...
(function () {
  ...
  FTKR.ICS.basic = {
    ...
    disableNumInput: +paramParse(
      parameters["Disable Material Number Input"] || 0
    ),
    varId: {
    ...
    treatMaterial: paramParse(
      parameters["Recipe Materials Treatment"] || "lost"
    ),
  ...
  var convertTextWidth = function (text) {
    ...
    if (/\x1bi\[(\d+)\]/i.test(text)) {
      ...
      text = text.toUpperCase().replace(/\x1bi\[(\d+)\]/gi, "");
    }
    if (/\x1bc\[(\d+)\]/i.test(text)) {
      text = text.toUpperCase().replace(/\x1bc\[(\d+)\]/gi, "");
    }
    if (/\x1b{/i.test(text)) {
      text = text.toUpperCase().replace(/\x1b{/gi, "");
    }
    if (/\x1b}/i.test(text)) {
      text = text.toUpperCase().replace(/\x1b}/gi, "");
    }
    if (text.match(/\x1blw\[(\d+),?([^\]]+)\]/i)) {
      tw += RegExp.$1;
      text = text.toUpperCase().replace(/\x1blw\[(\d+),?([^\]]+)\]/gi, "");
    }
    tw += textWidth(text);
    return tw;
  };
  ...
  Array.prototype.icsSoundType = function () {
    return this[FTKR.ICS.openType - 1] &&
      this[FTKR.ICS.openType - 1].name !== ""
      ? this[FTKR.ICS.openType - 1]
      : this[0];
  };
  ...
  var _ICS_Game_Interpreter_pluginCommand =
  ...
  Game_Interpreter.prototype.addRecipe = function (args) {
    ...
    if (item) {
      $gameParty.addRecipe(
        DataManager.itemDataClass(item),
        item.id,
        setArgNum(args[1])
      );
    } else if (dataClass) {
    ...
    for (var n = 1; n < group.length; n++) {
      ...
      if (this.isIcsCategory(obj)) {
        ...
        if (hasObjectMeta(obj, ["ICS ITEM", "ICS アイテム"])) {
          ...
          var category = readObjectMeta(obj, [
            "ICS CATEGORY",
            "ICS カテゴリー",
          ]);
          if (category) obj.ics.setDataClass(category);
        }
        var datas = readEntrapmentCodeToText(obj, [
          "ICS RECIPES",
          "ICS レシピ",
        ]);
  ...
  DataManager.icsCompositionNotetags = function (group) {
    for (var n = 1; n < group.length; n++) {
      ...
      if (!this.isIcsCategory(obj)) {
        ...
        obj.compositCount = 0;
        obj.ics.setCategory(
          readObjectMeta(obj, ["ICS CATEGORY", "ICS カテゴリー"])
        );
        obj.ics.setRank(
          Number(readObjectMeta(obj, ["ICS RANK", "ICS ランク"]))
        );
      ...
      for (var i = 0; i < datas.length; i++) {
        ...
        if (code) {
          switch (code[1].toUpperCase()) {
            case "ITEM":
            case "アイテム":
              var material = new Game_Material(
                0,
                "item",
                Number(code[2]),
                Number(match[2])
  ...
  DataManager.makeSaveContents = function () {
    ...
    if (FTKR.ICS.DATABASE_WEAPONS_NUMBER < $dataWeapons.length) {
      contents.icsWeaponDatas = $dataWeapons.slice(
        FTKR.ICS.DATABASE_WEAPONS_NUMBER
      );
    }
    if (FTKR.ICS.DATABASE_ARMORS_NUMBER < $dataArmors.length) {
      contents.icsArmorDatas = $dataArmors.slice(
        FTKR.ICS.DATABASE_ARMORS_NUMBER
      );
    }
    return contents;
  };
  ...
  DataManager.changeItemRank = function (item, addRank) {
    ...
    return items.length
      ? this.findSomeRank(items, item.ics.rank() + addRank)
      : item;
  };
  ...
  DataManager.searchCategoryItems = function (datas, category) {
    return datas.filter(function (item) {
      ...
      var notComp = hasObjectMeta(item, [
        "ICS 合成アイテム",
        "ICS COMPOSIT_ITEM",
      ]);
      return (
        !notComp &&
        (item.ics.category() === category ||
  ...
  Game_Material.prototype.initialize = function (
    category,
    dataClass,
    itemId,
    number
  ...
  Game_Material.prototype.matchMaterial = function (material) {
    return (
      (this.category() && this.matchMaterialCategory(material)) ||
      (!this.category() &&
        this.matchDataClass(material) &&
        this.matchItemId(material))
    );
  };
  ...
  Game_IcsRecipeBook.prototype.initialize = function (
    dataClass,
    itemId,
    typeId
  ) {
  ...
  Game_IcsRecipeBook.prototype.matchRecipe = function (
    dataClass,
    itemId,
    typeId
  ) {
  ...
  Game_Party.prototype.hasMaterial = function (material, number) {
    return (
      this.hasCategoryMaterial(material, number) ||
      this.hasNonCategoryMaterial(material, number)
    );
  };
  ...
  Game_Party.prototype.hasCategoryMaterial = function (material, number) {
    return (
      material.category() && this.numCategoryItem(material.category()) >= number
    );
    /*
  ...
  Game_Party.prototype.numCategoryItem = function (category) {
    ...
    this.allItems().forEach(function (item) {
      if (item && item.ics.category() == category)
        numItem += this.numItems(item);
    }, this);
    return numItem;
  };
  ...
  Game_Party.prototype.hasNonCategoryMaterial = function (material, number) {
    return (
      !material.category() &&
      this.numItems(material.item()) >= material.number() * number
    );
  };
  ...
  Game_Party.prototype.hasRequiredRecipeMaterials = function (item, number) {
    ...
    return (
      item &&
      item.ics.recipes().some(function (recipe) {
  ...
  Game_Party.prototype.hasRequiredRecipeIDMaterials = function (
    item,
    typeId,
    number
  ) {
  ...
  Game_Party.prototype.hasMaxRequiredRecipeIDMaterials = function (
    item,
    typeId
  ) {
    var number = 1;
  ...
  Window_Base.prototype.drawTextExAlignIndex = function (
    text,
    x,
    y,
    alignIndex
  ...
  Window_MenuCommand.prototype.isEnabledCompositionCmd = function (ext) {
    ...
    return (
      (cmd.enabled === 1 && cmd.switchId === 0) ||
      (cmd.switchId > 0 && $gameSwitches.value(cmd.switchId))
    );
  };
  ...
  Window_IcsCompsiTitle.prototype.refresh = function () {
    this.contents.clear();
    var text =
      FTKR.ICS.windowText.title || FTKR.ICS.compositTitle.format.icsType();
  ...
  Window_IcsCompsiTitle.prototype._refreshFrame = function () {
    if (FTKR.ICS.compositTitle.frame === "表示する(show)")
  ...
  Window_IcsCommand.prototype._refreshFrame = function () {
    if (FTKR.ICS.command.frame === "表示する(show)")
      Window.prototype._refreshFrame.call(this);
  };
  ...
  Window_IcsCommand.prototype.makeCommandList = function () {
    var commandList =
      FTKR.ICS.windowText.command || FTKR.ICS.command.list.icsType();
  ...
  Window_IcsItemList.prototype._refreshFrame = function () {
    if (FTKR.ICS.itemList.frame === "表示する(show)")
      Window.prototype._refreshFrame.call(this);
  };
  ...
  Window_IcsItemList.prototype.isEnabled = function (item, typeId) {
    ...
    return this._showResipe
      ? $gameParty.hasRequiredRecipeIDMaterials(item, typeId)
      : item;
  };
  ...
  Window_IcsItemList.prototype.drawItem = function (index) {
    ...
    if (item) {
      ...
      var typeId =
        this._showResipe && item.ics.recipes().length > 1
          ? this._typeId[index] + 1
          : "";
  ...
  Window_IcsItemList.prototype.drawItemName = function (
    item,
    x,
    y,
    width,
  ...
  Window_IcsCompsiSlotTitle.prototype.initialize = function (
    x,
    y,
    width,
    height
  ...
  Window_IcsCompsiSlotTitle.prototype.refresh = function () {
    this.contents.clear();
    var text =
      FTKR.ICS.windowText.slotTitle || FTKR.ICS.slotTitle.format.icsType();
  ...
  Window_IcsCompsiSlotTitle.prototype._refreshFrame = function () {
    if (FTKR.ICS.slotTitle.frame === "表示する(show)")
      Window.prototype._refreshFrame.call(this);
  };
  ...
  Window_IcsCompsiSlot.prototype._refreshFrame = function () {
    if (FTKR.ICS.slot.frame === "表示する(show)")
      Window.prototype._refreshFrame.call(this);
  };
  ...
  Window_IcsCompsiSlot.prototype.update = function () {
    ...
    if (this._compositionStateWindow)
      this._compositionStateWindow.setItems(this.materials());
  };
  ...
  Window_IcsNumber.prototype._refreshFrame = function () {
    if (FTKR.ICS.number.frame === "表示する(show)")
      Window.prototype._refreshFrame.call(this);
  };
  ...
  Window_IcsNumber.prototype.refresh = function () {
    ...
    if (this._compositionStateWindow) {
      if (
        FTKR.ICS.status.dispRecipe &&
        this._compositionStateWindow._showResipe &&
        this._compositionStateWindow._resipeItem
      ) {
  ...
  Window_IcsCompsiStateTitle.prototype.initialize = function (
    x,
    y,
    width,
    height
  ...
  Window_IcsCompsiStateTitle.prototype.refresh = function () {
    this.contents.clear();
    var text =
      FTKR.ICS.windowText.statusTitle || FTKR.ICS.statusTitle.format.icsType();
  ...
  Window_IcsCompsiStateTitle.prototype._refreshFrame = function () {
    if (FTKR.ICS.statusTitle.frame === "表示する(show)")
      Window.prototype._refreshFrame.call(this);
  };
  ...
  Window_IcsCompsiState.prototype._refreshFrame = function () {
    if (FTKR.ICS.status.frame === "表示する(show)")
      Window.prototype._refreshFrame.call(this);
  };
  ...
  Window_Base.prototype.drawItemNameNumber = function (
    item,
    number,
    x,
    y,
  ...
  Window_IcsCompsiState.prototype.refresh = function () {
    ...
    if (comp) {
      ...
      var has = $gameParty.hasRecipe(
        DataManager.itemDataClass(comp.item),
        comp.item.id,
        comp.typeId
      );
      if (has > 0 && number) {
        if (comp.item.ics.hasResultItem()) {
          ...
          if (recipe.failure()) {
            this.drawText(
              FTKR.ICS.result.failure.icsType(),
              0,
              y * (2 + dy),
              w / 2
            );
  ...
  Window_IcsCompsiState.prototype.drawResipeMaterials = function (
    item,
    typeId,
    x,
    y
    ...
    materials.forEach(function (material, i) {
      ...
      var numMat = (FTKR.ICS.status.recipeMaterial.text + "").format(
        material.number() * this._compNum,
        numItem
      );
  ...
  Window_IcsCompsiState.prototype.compositionsItem = function () {
    var comps = this.checkMaterials($dataItems).concat(
      this.checkMaterials($dataWeapons),
      this.checkMaterials($dataArmors)
    );
  ...
  Window_IcsCompsiState.prototype.matchRecipeMaterials = function (item) {
    ...
    return recipes.length
      ? { item: item, recipe: recipes[0], typeId: typeId }
      : null;
  };
  ...
  Window_IcsCompsiState.prototype.hasMaterials = function (materials) {
    ...
    if (FTKR.ICS.basic.match && materials.length !== slotMaterials.length)
      return false;
  ...
  Window_IcsCompsiState.prototype.hasMaterial = function (
    material,
    slotMaterials
  ) {
  ...
  Window_IcsConfTitle.prototype._refreshFrame = function () {
    if (FTKR.ICS.conf.frame === "表示する(show)")
      Window.prototype._refreshFrame.call(this);
  };
  ...
  Window_IcsEndConfTitle.prototype._refreshFrame = function () {
    if (FTKR.ICS.endConf.frame === "表示する(show)")
      Window.prototype._refreshFrame.call(this);
  };
  ...
  Window_IcsResult.prototype._refreshFrame = function () {
    if (FTKR.ICS.result.frame === "表示する(show)")
      Window.prototype._refreshFrame.call(this);
  };
  ...
  Window_IcsResultConf.prototype.makeItemList = function () {
    this._data = [{ dicision: true, disp: FTKR.ICS.result.okFormat.icsType() }];
  };
  ...
  Scene_ICS.prototype.createCompositionSlotTitleWindow = function () {
    ...
    var wh = this._helpWindow.fittingHeight(1);
    this._compositionSlotTitleWindow = new Window_IcsCompsiSlotTitle(
      0,
      wy,
      ww,
      wh
  ...
  Scene_ICS.prototype.createCompositionSlotWindow = function () {
    var wy =
      this._compositionSlotTitleWindow.y +
      this._compositionSlotTitleWindow.height;
    var ww = Graphics.boxWidth / 2;
  ...
  Scene_ICS.prototype.createCompositionStateTitleWindow = function () {
    ...
    var ww = Graphics.boxWidth / 2;
    var wh = this._helpWindow.fittingHeight(1);
    this._compositionStateTitleWindow = new Window_IcsCompsiStateTitle(
      wx,
      wy,
      ww,
      wh
  ...
  Scene_ICS.prototype.createCompositionStateWindow = function () {
    var wx = this._compositionSlotWindow.width;
    var wy =
      this._compositionStateTitleWindow.y +
      this._compositionStateTitleWindow.height;
    var ww = Graphics.boxWidth / 2;
    var wh = Graphics.boxHeight - wy;
  ...
  Scene_ICS.prototype.onItemOk = function () {
    if (this._compositionSlotWindow._itemCount < 5) {
      if (this._itemWindow._showResipe) {
        ...
        var max = $gameParty.hasMaxRequiredRecipeIDMaterials(
          this.item(),
          this._itemWindow.typeId()
        );
  ...
  Scene_ICS.prototype.setSlotItem = function (item, number) {
    ...
    if (compsiSlot.length) {
      ...
      csw._slots[csw._itemCount] = new Game_Material(
        "",
        DataManager.itemDataClass(item),
        item.id,
        number
  ...
  Scene_ICS.prototype.refreshWindow = function () {
    ...
    this._compositionStateWindow.setItems(
      this._compositionSlotWindow.materials()
    );
    this._categoryWindow.refreshCom(this._compositionSlotWindow.materials());
  };
  ...
  Scene_ICS.prototype.isSpecialComp = function (item) {
    return (
      item &&
      item.wtypeId === FTKR.ICS.basic.categoryId &&
  ...
  Scene_ICS.prototype.composition = function (sound) {
    ...
    if (item) {
      ...
      if (
        this.successApply(judg, getItem, item.ics.recipe(typeId), sound) &&
        composit._learnRecipe
      ) {
  ...
  Scene_ICS.prototype.setItemTraits = function (baseItem, addItem, number) {
    ...
    traits = traits.concat(
      addItem.traits.map(function (trait) {
  ...
  Scene_ICS.prototype.successCorrection = function (success) {
    ...
    switch (match[1].toUpperCase()) {
      ...
      case "CHANGE_RANK":
        return {
          code: Window_Base.SUCCESS_CORRECTION_RANK,
          value: Number(match[2]),
        };
      case "カテゴリー変更":
      case "CHANGE_CATEGORY":
        return {
          code: Window_Base.SUCCESS_CORRECTION_CATEGORY,
          value: match[2],
        };
      case "生成数変更":
      case "CHANGE_NUMBER":
        return {
          code: Window_Base.SUCCESS_CORRECTION_NUMBER,
          value: Number(match[2]),
        };
      case "アイテム変更":
      case "CHANGE_ITEM":
  ...
  Scene_ICS.prototype.successJudg = function (item, typeId) {
    ...
    var difficulty = recipe.difficulty();
    var rate = param ? this.calcSuccessRate(difficulty, param) : difficulty;
    ...
    var failure =
      recipe.failure() || item.ics.hasResultItemFailure()
        ? rate.failure.clamp(0, max - great - success)
        : 0;
  ...
  Scene_ICS.prototype.setCorrection = function (correct, getItem) {
    ...
    switch (correct.code) {
      ...
      case Window_Base.SUCCESS_CORRECTION_CATEGORY:
        getItem.item = DataManager.changeItemCategory(
          getItem.item,
          correct.value
        );
        break;
