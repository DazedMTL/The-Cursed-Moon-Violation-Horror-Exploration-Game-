...
(() => {
  ...
  const settings = {
    ...
    prohibitLineBreakAfter: String(
      pluginParameters.prohibitLineBreakAfter || "([｛〔〈《「『【〘〖〝‘“｟«"
    ),
    ignoreAutoLineBreakWindows: JSON.parse(
      pluginParameters.ignoreAutoLineBreakWindows || "[]"
    ).map((e) => {
      return String(e || "");
    }),
  ...
  function Window_AutoLineBreakMixIn(windowClass) {
    windowClass.ignoreAutoLineBreak = function () {
      return settings.ignoreAutoLineBreakWindows.includes(
