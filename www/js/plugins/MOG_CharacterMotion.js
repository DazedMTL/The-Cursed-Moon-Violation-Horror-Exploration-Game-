...
// * PluginCommand
//==============================
var _alias_mog_charmotion_pluginCommand =
...
Game_Interpreter.prototype.checkFollowerCharEffects = function (command, args) {
  ...
  if (npk === 0) {
    if (bparty_id === 0) {
      $gamePlayer._breathData[0] = Number(args[3]);
    }
    if (gparty_id === 0) {
      $gamePlayer._ghostData[0] = Number(args[3]);
    }
    if (fparty_id === 0) {
      $gamePlayer._floatData[0] = 1;
    }
    if (sparty_id === 0) {
      $gamePlayer._swingData[0] = 1;
    }
    if (rparty_id === 0) {
      $gamePlayer._rotationData[1] = Number(args[3]);
    }
    if (colparty_id === 0) {
      $gamePlayer._collapseData[0] = Number(args[3]);
    }
    if (shparty_id === 0) {
      $gamePlayer._shakeData[0] = Number(args[3]);
    }
    if (zparty_id === 0) {
    ...
    if (cparty_id === 0) {
      $gamePlayer.charEffectsClear();
    }
  }
  if (npk === 1) {
    var index = 0;
    $gamePlayer.followers().forEach(function (follower) {
      if (index === bparty_id - 1) {
        follower._breathData[0] = Number(args[3]);
      }
      if (index === gparty_id - 1) {
        follower._ghostData[0] = Number(args[3]);
      }
      if (index === fparty_id - 1) {
        follower._floatData[0] = 1;
      }
      if (index === sparty_id - 1) {
        follower._swingData[0] = 1;
      }
      if (index === rparty_id - 1) {
        follower._rotationData[1] = Number(args[3]);
      }
      if (index === shparty_id - 1) {
        follower._shakeData[0] = Number(args[3]);
      }
      if (index === colparty_id - 1) {
        follower._collapseData[0] = Number(args[3]);
      }
      if (index === zparty_id - 1) {
        follower._zoomData[2] = Number(args[3]);
        follower._zoomData[3] = Number(args[3]);
      }
      if (index === cparty_id - 1) {
        follower.charEffectsClear();
      }
      index++;
    }, this);
  }
...
Game_Interpreter.prototype.checkEventsCharEffects = function (command, args) {
  var nck = false;
  if (command === "breath_mode_event_id") {
    var bevent_id = Number(args[1]);
    nck = true;
  }
  if (command === "ghost_mode_event_id") {
    var gevent_id = Number(args[1]);
    nck = true;
  }
  if (command === "float_mode_event_id") {
    var fevent_id = Number(args[1]);
    nck = true;
  }
  if (command === "swing_mode_event_id") {
    var sevent_id = Number(args[1]);
    nck = true;
  }
  //	if (command === "collpase_effect_event_id")  {var colevent_id = Number(args[1]); nck = true};
  if (command === "collapse_effect_event_id") {
    var colevent_id = Number(args[1]);
    nck = true;
  }
  if (command === "zoom_event_id") {
    var zevent_id = Number(args[1]);
    nck = true;
  }
  if (command === "rotation_event_id") {
    var revent_id = Number(args[1]);
    nck = true;
  }
  if (command === "shake_effects_event_id") {
    var shevent_id = Number(args[1]);
    nck = true;
  }
  if (command === "clear_effects_event_id") {
    var cevent_id = Number(args[1]);
    nck = true;
  }
  if (nck) {
    $gameMap.events().forEach(function (event) {
      if (event.eventId() === bevent_id) {
        event._breathData[0] = Number(args[3]);
      }
      if (event.eventId() === gevent_id) {
        event._ghostData[0] = Number(args[3]);
      }
      if (event.eventId() === fevent_id) {
        event._floatData[0] = 1;
      }
      if (event.eventId() === sevent_id) {
        event._swingData[0] = 1;
      }
      if (event.eventId() === revent_id) {
        event._rotationData[1] = Number(args[3]);
      }
      if (event.eventId() === shevent_id) {
        event._shakeData[0] = Number(args[3]);
      }
      if (event.eventId() === colevent_id) {
        event._collapseData[0] = Number(args[3]);
      }
      if (event.eventId() === zevent_id) {
        event._zoomData[2] = Number(args[3]);
        event._zoomData[3] = Number(args[3]);
      }
      if (event.eventId() === cevent_id) {
        event.charEffectsClear();
      }
    }, this);
  }
};
